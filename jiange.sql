...
Host - 5.0.41-community-nt : Database - jiange
*********************************************************************
...
create database if not exists `jiange`;
...
/*Table structure for table `jg_db_version` */

DROP TABLE IF EXISTS `jg_db_version`;

CREATE TABLE `jg_db_version` (
  `db_version` int(11) NOT NULL default '10000' COMMENT '数据库版本',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='铸鼎装备物品表';
...
/*Table structure for table `jg_item_rank` */
...
/*!50001 CREATE TABLE `jg_item_rank` (
  ...
  `equip_strengthen_lv` tinyint(3) unsigned NOT NULL default '0' COMMENT '装备强化等级',
  `equip_enchanted_lv` tinyint(3) unsigned NOT NULL default '0' COMMENT '装备注魔等级',
  `item_lv` tinyint(3) unsigned NOT NULL default '0' COMMENT '物品需求等级',
  `player_name` char(20) character set utf8 collate utf8_unicode_ci NOT NULL COMMENT '姓名',
  `player_lv` tinyint(3) NOT NULL default '0' COMMENT '等级'
) ENGINE=InnoDB DEFAULT CHARSET=gbk */;
...
CREATE TABLE `jg_player_attr` (
  ...
  `guide_triggled_state` bigint(20) unsigned NOT NULL default '0' COMMENT '玩家指引触发状态',
  `map_monster_pg` tinytext COMMENT '地图打怪进度字典地图ID 对于进度 如果没有对于的地图ID表示该地图没打过',
  PRIMARY KEY  (`player_id`)
...
/*Table structure for table `jg_player_view` */
...
/*!50001 CREATE TABLE `jg_player_view` (
  ...
  `lv` tinyint(3) default NULL COMMENT '等级',
  `exp` int(10) default NULL COMMENT '经验',
  `strength` float default NULL COMMENT '力量',
  `intelligence` float default NULL COMMENT '学识',
  `agile` float default NULL COMMENT '身法',
  `hp` int(10) default NULL COMMENT '生命',
  `max_hp` int(10) default NULL COMMENT '最大生命',
  `mp` int(10) default NULL COMMENT '法力',
  `max_mp` int(10) default NULL COMMENT '最大法力',
  `phy_atk` int(10) default NULL COMMENT '武术攻击',
  `phy_def` int(10) default NULL COMMENT '武术防御',
  `mag_atk` int(10) default NULL COMMENT '法术攻击',
  `mag_def` int(10) default NULL COMMENT '法术防御',
  `phy_crit_point` int(10) default NULL COMMENT '武爆值（能力）',
  `phy_crit` int(5) default NULL COMMENT '武术暴击率',
  `mag_crit_point` int(10) default NULL COMMENT '法爆值（能力）',
  `mag_crit` int(5) default NULL COMMENT '法术暴击率',
  `hit_point` int(10) default NULL COMMENT '命中值',
  `hit` int(5) default NULL COMMENT '命中率',
  `dodge_point` int(10) default NULL COMMENT '闪避值（能力）',
  `dodge` int(5) default NULL COMMENT '闪避率',
  `rebound` int(5) default NULL COMMENT '反弹',
  `absorbed_hp` int(5) default NULL COMMENT '吸血',
  `absorbed_mp` int(5) default NULL COMMENT '吸蓝',
  `speed` int(10) default NULL COMMENT '先手速度',
  `remain_point` int(10) default NULL COMMENT '剩余点数',
  `status` tinyint(3) default NULL COMMENT '状态',
  `money` int(10) default NULL COMMENT '金钱',
  `coupon` int(10) default NULL COMMENT '礼券',
  `vigour` int(10) default NULL COMMENT '活力',
  `zhen_qi` int(10) default NULL COMMENT '真气',
  `map_id` int(10) default NULL COMMENT '所在地图可以是FB',
  `last_map_id` int(10) default NULL COMMENT '上次所在地图一定不会是副本',
  `honor_point` int(10) default NULL COMMENT '荣誉点数',
  `sin_point` int(10) default NULL COMMENT '罪恶点数',
  `strengthen_num` int(10) default NULL COMMENT '强化总次数(每天初始为0)',
  `strengthen_time` int(10) default NULL COMMENT '强化CD总时间',
  `strengthen_last_time` int(10) default NULL COMMENT '最后强化时间',
  `strengthen_status` tinyint(3) default NULL COMMENT '连续强化状态 0：可连续强化 1：不可强化',
  `strengthen_rate` tinyint(3) default NULL COMMENT '强化成功率(每天初始60%)',
  `skill_series` tinyint(3) default NULL COMMENT '启用的技能系列',
  `map_point_x` int(10) default NULL COMMENT '玩家所在地图的坐标位置(x坐标)',
  `map_point_y` int(10) default NULL COMMENT '玩家所在地图的坐标位置(y坐标)',
  `friend_validation` tinyint(1) default NULL COMMENT '加好友是否需要验证',
  `society_contribute` int(10) default NULL COMMENT '帮会贡献  ',
  `today_token_count` int(10) default NULL COMMENT '今天英雄令数量',
  `yestoday_token_count` int(10) default NULL COMMENT '昨天英雄令数量',
  `challenger_refreshed` tinyint(1) default NULL COMMENT '列表是否刷新过延迟刷新',
  `challenger_data` text character set utf8 collate utf8_unicode_ci COMMENT '英雄令对手数据',
  `aready_fight` tinyint(1) default NULL COMMENT '英雄令活动当天是否战斗过',
  `hero_token_prize_fetched` tinyint(1) default NULL COMMENT '是否领取过当天的英雄令',
  `latest_token_count` int(10) default NULL COMMENT '最近一天获得英雄令的数量 可能是昨天的也可能是很久以前的如果没有上线的话',
  `total_token_count` int(10) default NULL COMMENT '获得的英雄令的总数',
  `hero_token_beated` int(10) default NULL COMMENT '当天已经战胜过的次数',
  `today_logined` tinyint(1) default NULL COMMENT '今天是否登陆过',
  `nation_id` tinyint(3) default NULL COMMENT '国家ID'
) ENGINE=InnoDB DEFAULT CHARSET=gbk */;
...
CREATE TABLE `jg_society_member` (
  ...
  `paid_flag` tinyint(1) NOT NULL default '0' COMMENT '是否有工资可以领取',
  `remain_donate_num` int(11) NOT NULL default '0' COMMENT '剩余捐献次数',
  `join_society_war` tinyint(1) NOT NULL default '0' COMMENT '是否参加帮会大乱战',
